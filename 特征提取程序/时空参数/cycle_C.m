...
maindir='C:\Users\jxy\Desktop\CSM数据库\HC'; %读取主文件夹（按照实际情况修改）
subdir  = dir( maindir );%读取子文件夹
i=1;
for k=3:92
    subdirpath = fullfile( maindir, subdir(k).name, '1' );%进入子文件夹内部路径，按照自己的文件路径即可
...
xlswrite('...... .xlsx',cycle_mean1,'time','B2'); %%%xlswrite('数据表格路径',cycle_mean1,'表格名称，可自定义，也可按照该特征在所提供的示例表格中的位置进行写入','写入数据在表格中的起始位置')  此行代码表示写入CSM病人数据
xlswrite('...... .xlsx',cycle_mean2,'time','B92');%%%此行代码表示写入正常人数据
